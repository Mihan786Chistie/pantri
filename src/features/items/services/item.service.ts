...
export async function getCategories(): Promise<string[]> {
    ...
    const records = await database.get<any>('categories').query(
        ...
        await database.write(async () => {
            for (const name of defaults) {
                await database.get<any>('categories').create((record: any) => {
                    ...
                    record.name = name;
                    record.userId = userId; // Seed with current user's ID so they can manage it
                });
            }
        });
        return defaults;
...
export async function createCategory(name: string): Promise<string> {
    ...
    const existing = await database.get<any>('categories').query(
        Q.where('name', Q.like(trimmed))
    ).fetch();
...
export async function updateCategory(oldName: string, newName: string): Promise<void> {
    ...
    await database.write(async () => {
        const catRecords = await database.get<any>('categories').query(
            Q.where('name', trimmedOld),
            Q.where('user_id', userId)
        ).fetch();
        ...
        const itemRecords = await database.get<any>('items').query(
            Q.where('category', trimmedOld),
            Q.where('user_id', userId)
        ).fetch();
